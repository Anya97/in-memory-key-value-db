...
func TestPipeline_Execute(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErrContains: "set error: boom",
		},
		{
		...
		},
		{
			...
			wantErrContains: "delete error: missing",
		},
		{
			name:            "parse error surfaced",
			input:           "UNKNOWN",
			fake:            &fakeStorage{},
			wantErrContains: "parse error: unknown command",
			assertCalls: func(t *testing.T, f *fakeStorage) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			p := NewPipeline(compute.NewCompute(), tt.fake)

			var out string
			var err error
			if tt.wantOut != "" {
				out = captureStdout(func() { err = p.Execute(tt.input) })
			} else {
				err = p.Execute(tt.input)
			}
